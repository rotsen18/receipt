...
from telegram_bot.handlers.onboarding.static_text import catalogue_button_name
...
def make_main_menu_keyboard(user: TelegramUser) -> ReplyKeyboardMarkup:
    catalogue = KeyboardButton(text=catalogue_button_name)
    buttons = [[catalogue]]
    ...
    if user.is_telegram_admin or user.is_telegram_moderator:
        buttons.append([receipts, categories])
